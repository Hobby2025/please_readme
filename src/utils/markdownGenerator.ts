...
export function generateMarkdown(profile: ProfileInfo): string {
  ...
  if (profile.useDarkLightMode) {
    ...
    markdown += `  <picture>\n`;
    markdown += `    <source media="(prefers-color-scheme: dark)" srcset="https://please-readme.vercel.app/api/github-card?username=${encodeURIComponent(profile.username)}&theme=dark&name=${encodeURIComponent(profile.name)}&bio=${encodeURIComponent(profile.bio)}&t=${Date.now()}">\n`;
    markdown += `    <source media="(prefers-color-scheme: light)" srcset="https://please-readme.vercel.app/api/github-card?username=${encodeURIComponent(profile.username)}&theme=light&name=${encodeURIComponent(profile.name)}&bio=${encodeURIComponent(profile.bio)}&t=${Date.now()}">\n`;
    markdown += `    <img alt="GitHub Profile" src="https://please-readme.vercel.app/api/github-card?username=${encodeURIComponent(profile.username)}&theme=light&name=${encodeURIComponent(profile.name)}&bio=${encodeURIComponent(profile.bio)}&t=${Date.now()}">\n`;
    markdown += `  </picture>\n`;
    markdown += `</div>\n\n`;
  } else {
    // 단일 테마
    markdown += `<div align="center">\n`;
    markdown += `  <img src="https://please-readme.vercel.app/api/github-card?username=${encodeURIComponent(profile.username)}&theme=${profile.theme || 'light'}&name=${encodeURIComponent(profile.name)}&bio=${encodeURIComponent(profile.bio)}&t=${Date.now()}" alt="GitHub Profile" />\n`;
    markdown += `</div>\n\n`;
  }
